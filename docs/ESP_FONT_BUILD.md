...
- 紧凑模式：80×27，单列 12×24 px，字体 20 px。
- 大字模式：64×20，单列 16×32 px，字体 26 px。
- 正文限于上方 648 像素，不覆盖底部 120 像素状态导航栏。
...
缩小时采用区域最大 alpha 采样，以保留细笔画，视觉上可能比灰度插值偏重。所有写入同时受单元与帧缓冲边界限制。各字号的度量独立缓存；字形缓存命中无需切换 FreeType 当前尺寸，绘制基线使用对应字号的缓存度量。位图预算仍为 512 KiB，容量压力使用有界的 second-chance（第二次机会）逐项淘汰，而非一次清空全部字形。删除条目会修复哈希探测链；仍存活位图的地址不变，但后续加载缺失字形可能淘汰先前返回的位图，调用者必须及时消费或复制。

这些规则并不提供完整 Unicode 排版、组合字形、emoji 或自动字体回退。缺失码点可能由 FreeType 显示 `.notdef`，所以加载成功不能替代字符覆盖检查。

## 设置滚动条带裁剪

`ttf_draw_text_clipped()` 与原有 `ttf_draw_text()` 共用栅格绘制实现，增加半开区间 `[clip_y0, clip_y1)` 的垂直裁剪。普通接口仍按整个传入帧缓冲高度裁剪；新接口只写入合法条带，保留原来的字形度量、基线、字距和返回宽度。空条带不写像素；负下界、倒置区间或超出帧缓冲的上界直接拒绝。

设置连续滚动先移动已有正文像素，再通过此接口绘制新露出的行；没有新增字体位图预算或完整 UI 帧缓冲。裁剪是栅格化优化，移动后的整个正文仍需提交显示。
...
- 1–255 字号的缓存度量、预热后零尺寸切换／零字形加载，512 KiB 位图预算、随机压力、跨表尾删除链修复、存活位图地址和分配失败恢复；这些用例开启 AddressSanitizer 与 UndefinedBehaviorSanitizer。
- 启动顺序和 UI 调用路径的源码集成检查。
- 文本条带裁剪分别使用合成字体与真实字体，每种字体检查 33,462 组组合：13 个字号、逐行与空条带、跨边界字形和 8 类非法范围。合法范围内逐像素等于完整绘制，范围外与保护区保持不变，并检查返回宽度；开启 AddressSanitizer 与 UndefinedBehaviorSanitizer。

终端单元的原生逐像素测试集中在 20 px 字体、12／24 px 宽且 24 px 高的单元；独立文本裁剪测试扩展到 13 个字号，仍不等于穷尽所有 UI 字号或大字模式。缺失编译器、fontTools 或静态库时用例会跳过；测试结果需同时记录跳过原因。
