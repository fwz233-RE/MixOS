...
- **界面与终端**：1024×768 / 400 PPI RGB 屏幕、十二种手工 Material 3 风格颜色预设、中英文界面、FreeType 字体及 Linux 终端文本渲染。
...
显示仅保留稳定 RGB565／26 MHz 配置：1024×768、两块驱动帧缓冲与每块 16 行的 DMA 中转缓冲。高刷与 RGB332 实验实现、配置开关和构建覆盖文件已移除；稳定双缓冲、局部更新、渐变和换帧等待期间的触控采样仍保留。
