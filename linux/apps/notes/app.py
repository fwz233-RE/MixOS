...
back first. Ctrl-N, Ctrl-D and Ctrl-Q do the same three things and always
arrive. The letters still work for a device with no input method installed;
...
class Notes:
    ...
    def _draw_list(self) -> None:
        ...
        self.screen.put(1, 0, '笔记', INK, 4, tui.BOLD)
        count = f'{len(self.notes)} 条'
        self.screen.put(max(1, width - tui.text_width(count) - 1), 0, count, INK, 4)
        self.screen.set_cursor(None)

        body = height - 3
        if not self.notes:
            self.screen.put(2, 2, '还没有笔记。按 Ctrl+N 新建。', DIM)
            return
        ...
        for offset in range(min(body, len(self.notes))):
            ...
            y = 1 + offset
            if chosen:
                self.screen.fill(0, y, width, 1, bg=4)
            stamp = note.when()
            self.screen.put(1, y, stamp, WARN if chosen else DIM, 4 if chosen else -1)
            self.screen.put(1 + tui.text_width(stamp) + 1, y,
                            tui.truncate(note.title(),
                                         width - tui.text_width(stamp) - 3),
                            INK, 4 if chosen else -1,
                            tui.BOLD if chosen else 0)

    def _draw_editor(self) -> None:
        width, height = self.screen.columns, self.screen.rows
        self.screen.fill(0, 0, width, 1, bg=4)
        title = self.buffer.lines[0].strip() or self.name
        self.screen.put(1, 0, tui.truncate(title, width - 12), INK, 4, tui.BOLD)
        mark = '●' if self.buffer.modified else '○'
        self.screen.put(width - 2, 0, mark, WARN if self.buffer.modified else DIM, 4)
        ...
        for offset in range(body):
            ...
            if index >= len(rows):
                break
            self.screen.put(1, 1 + offset, rows[index][2], INK)
        if self.recording:
            self._meter(height)
        self.screen.set_cursor(1 + cursor_cells, 1 + cursor_row - self.top)

    def _meter(self, height: int) -> None:
        width = self.screen.columns
        caption = f' 录音 {self.recorder.seconds:4.1f}s '
        self.screen.fill(0, height - 2, width, 1, bg=1)
        self.screen.put(1, height - 2, caption, INK, 1, tui.BOLD)
        bar = max(0, width - tui.text_width(caption) - 2)
        filled = int(bar * audio.meter(self.recorder.level))
        self.screen.put(1 + tui.text_width(caption), height - 2, '█' * filled, GOOD, 1)

    def _footer(self) -> None:
        width, height = self.screen.columns, self.screen.rows
        if self.status and not (self.mode == 'edit' and self.recording):
            colour = {'info': DIM, 'warn': WARN, 'bad': BAD, 'good': GOOD}[self.status_kind]
        ...
        if self.mode == 'list':
            ...
            keys = '↑↓ 选择   Enter 打开   ^N 新建   ^D 删除   ^Q 退出'
        else:
            # Which language the recogniser is set to belongs on screen: it is
            # the difference between "recognition is broken" and "recognition
            # is listening for the other language".
            keys = (f'Ctrl+R 语音:{language_label(self.speech_language)}  '
                    f'Ctrl+T 换  Ctrl+S 保存  Ctrl+Q 返回')
        self.screen.fill(0, height - 1, width, 1, bg=0)
        self.screen.put(1, height - 1, tui.truncate(keys, width - 2), DIM, 0)
    ...
    def _list_key(self, name: str) -> bool:
        if self.confirm_delete:
            ...
            if name in ('y', 'Y', 'enter') and self.notes:
                ...
                self.say('')
            return True
        if name in ('q', 'Q', 'ctrl-q', 'ctrl-c', 'escape'):
            return False
        if name == 'up':
    ...
    def _edit_key(self, name: str) -> bool:
        ...
        if name == 'ctrl-q' or name == 'escape':
            ...
            buffer.move_visual(rows, -(self.screen.rows - 4))
        elif name == 'pagedown':
            buffer.move_visual(rows, self.screen.rows - 4)
        elif name == 'tab':
            buffer.insert_text('  ')
