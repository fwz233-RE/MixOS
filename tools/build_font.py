...
def validate_paths(source: Path, output: Path, manifest: Path, ui_source: Path) -> tuple[Path, ...]:
    paths = tuple(Path(p).resolve() for p in (source, output, manifest, ui_source))
...
def build(source: Path, output: Path, manifest: Path, *, ui_source: Path = DEFAULT_UI) -> dict[str, object]:
    source, output, manifest, ui_source = validate_paths(source, output, manifest, ui_source)
...
def main() -> int:
    ...
    try:
        record = build(args.source, args.output, manifest, ui_source=args.ui_source)
    ...
    print(json.dumps({k: record[k] for k in ("status", "output", "output_sha256", "output_bytes",
                     "required_codepoints", "included_codepoints", "gb2312_included", "glyph_aliases",
                     "source_missing_optional_codepoints")}, indent=2))
    return 0
